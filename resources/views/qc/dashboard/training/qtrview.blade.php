            ...
            <div class="col-md-4">
                <table class="table">
                    <div class="row ms-3 bg-gradien-light  pt-3 text-center item-center justify-content-center">
                        <div class="col-md-2">
                            <a href="/qc/training/qtrprint/{{ $view->id }}" target="_blank">
                                <button type="button" class="btn btn-sm btn-info" id="btn-tambah">Print </button>
                            </a>
                        </div>
                        <div class="col-md-2">
                            @if($view->approve == 'YES'  )
                            <a href="{{ route('training.qtrdelete', $view->id) }}" onclick="return confirm('Apakah Anda Yakin ?');" class="btn btn-sm btn-danger disabled">Delete</a>
                            @else  
                            <a href="{{ route('training.qtrdelete', $view->id) }}" onclick="return confirm('Apakah Anda Yakin ?');" class="btn btn-sm btn-danger">Delete</a>
                            @endif

                        </div>
                        <div class="col-md-2 ms-2">
                            @if($view->approve == 'YES'  )
                            <a  href="{{ route('training.qtredit', $view->id) }}" class="btn btn-sm btn-primary disabled">Edit</a>
                            @else  
                            <a  href="{{ route('training.qtredit', $view->id) }}" class="btn btn-sm btn-primary ">Edit</a>
                            @endif
                        </div>
                    </div>
                    <tr>
