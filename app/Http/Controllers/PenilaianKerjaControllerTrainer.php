...
class PenilaianKerjaControllerTrainer extends Controller
{
    public function index()
    {
        $namaprofile = Auth::user();
        $peg = pegawai::where('nama_asli', $namaprofile->name)->first();
        $akses = akses_users::where('iduser', $namaprofile->id)->pluck('bagian');
        $aksesbg = akses_users::where('iduser', $namaprofile->id)
            ->pluck('bagian')
            ->implode(', ');

        $pkinerja = Pkinerja::join('pkinerja_bs', 'pkinerjas.kode', '=', 'pkinerja_bs.kode')
            ->whereIn('pkinerjas.bagian', $akses)
            ->whereYear('pkinerjas.created_at', '=', now()->year)
        ...
        $jumlahsemua = pegawai::whereIn('bagian', $akses)
        ...
        return view('hr.dashboard.training.trainer.penilaiankerja.index', compact('namaprofile', 'pkinerja', 'jumlahsemua', 'jumlahygsudah', 'akses', 'aksesbg'));
    }
    ...
    public function create()
    {
        ...
        return view('hr.dashboard.training.trainer.penilaiankerja.create', compact('namaprofile', 'peg', 'kode', 'kompeten', 'kompeten2'));
    }

    public function kelengkapanpegawaitr($no_payroll)
    {
        $employee = pegawai::where('no_payroll', $no_payroll)->first();
        return response()->json($employee);
    }
    public function kelengkapandataperbagian($bagian)
    {
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'nilai1' => '', // Hapus '*' dari 'nilai1'
            'nilai2' => '', // Hapus '*' dari 'nilai2'
            'nilai3' => '', // Hapus '*' dari 'nilai3'
            'perf_faktor.*' => 'required', // Tambahkan '*' kembali di sini
            'nilai1.*' => '', // Tambahkan '*' kembali di sini
            'nilai2.*' => '', // Tambahkan '*' kembali di sini
            'nilai3.*' => '', // Tambahkan '*' kembali di sini
            'major_job' => '', // Hapus '*' dari 'perf_faktor'
        ...
        if ($pkinerja) {
            ...
            if ($akhir <= $periodeAkhirCarbon) {
                ...
                return redirect()
                    ->route('tr.hr.penilaiankerja.edit', ['id' => $pkinerja->id])
                    ->with([
        ...
        return redirect()
            ->route('tr.hr.penilaiankerja.index')
    ...
    public function view($id)
    {
        ...
        return view('hr.dashboard.training.trainer.penilaiankerja.view', compact('namaprofile', 'pkinerja', 'peg', 'pkinerja_b', 'kompeten'));
    }
    public function print($id)
    {
        ...
        $pdf = Pdf::loadview('hr.dashboard.training.trainer.penilaiankerja.print', compact('pkinerja', 'peg', 'pkinerja_b', 'kompeten', 'kontrak_period'));
    ...
    public function edit($id)
    {
        ...
        return view('hr.dashboard.training.trainer.penilaiankerja.edit', compact('penilai', 'namaprofile', 'pkinerja', 'peg', 'pkinerja_b', 'kompeten', 'kode', 'kompeten2'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Update record PenilaianKerja dengan data yang dikirim melalui form
        ...
        foreach ($validatedData['perf_faktor'] as $index => $perfFaktor) {
            ...
            if (!$existingRecord->un1) {
                $updateData['un1'] = $un;
            } elseif (!$existingRecord->un2 && array_sum(array_map('intval', explode(',', $updateData['nilai2']))) > 0) {
                $updateData['un2'] = $un;
            } elseif (!$existingRecord->un3 && array_sum(array_map('intval', explode(',', $updateData['nilai3']))) > 0) {
                $updateData['un3'] = $un;
            }
    ...
    public function laporan()
    {
        ...
        return view('hr.dashboard.training.trainer.penilaiankerjareport.laporan', compact('periodes','namaprofile'));
    }
    ...
    public function listLaporan(Request $request)
    {
        ...
        if ($request->report_type == 'pdforprint') {
            $pdf = Pdf::loadview('hr.dashboard.training.trainer.penilaiankerjareport.printLaporanPdf', ['pkinerja' => $pkinerja, 'periode' => $periode]);
